...
static inline void extioi_init_hart(void)
{
    ...
    iocsr_writed((0x1UL << UART0_IRQ) | (0x1UL << KEYBOARD_IRQ), LOONGARCH_IOCSR_EXTIOI_EN_BASE);

    /* extioi[31:0] map to cpu irq pin INT1, other to INT0 */
    iocsr_writed(0x01UL,LOONGARCH_IOCSR_EXTIOI_IPMAP_BASE);

    /* extioi IRQ 0-7 route to core 0, use node type 0 */
    iocsr_writed(0x0UL,LOONGARCH_IOCSR_EXTIOI_ROUTE_BASE);

    /* nodetype0 set to 1, always trigger at node 0 */
    iocsr_writed(0x1,LOONGARCH_IOCSR_EXTIOI_NODEMAP_BASE);
}
